...
- **The planned feature backlog is empty.** `014` — the last ticket on the analytics-to-profile
  spine — is now **complete** (see Done below), so nothing feature-scoped remains. The only ticket
  left in `backlog/` is `099` (split the shared dev/prod database), a deliberately **post-deadline**
  infra chore. `036` (popularity regression) was **cut entirely** —
  [ADR-0016](../decisions/adr/0016-cut-second-regression-model.md): no dataset supports a defensible
  popularity regression, and popularity isn't a stable regression target anyway.
