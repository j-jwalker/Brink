...
**Ready to start now** (all `blocked_by` merged, as of T34):
...
- **Done:** `034` — K-means training + `ModelArtifact` export (Jonah). Trains on the full local
  ...
  requirements). Unblocks `033`, now ready too — but see its ticket's flagged new prerequisite: the
  10-feature widening means `Track`'s schema needs 5 more columns before real-user inference can
  work.
