...
**Ready to start now** (all `blocked_by` merged, as of the T36 cut):
- No remaining enablement-wave blockers. `033` and `035` are complete; `014` is the only ticket
  left on the analytics-to-profile spine, unblocked and ready for backend work. `036` (popularity
  regression) was **cut entirely** — [ADR-0016](../decisions/adr/0016-cut-second-regression-model.md):
  no dataset supports a defensible popularity regression, and popularity isn't a stable regression
  target anyway. `038` (pipeline orchestration) no longer depends on it, dropping `036` from its
  `blocked_by` — both its remaining blockers (`031`, `034`) are merged, so `038` is unblocked too.
